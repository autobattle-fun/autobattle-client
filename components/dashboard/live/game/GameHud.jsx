    ...
    <div className="space-y-2 rounded-2xl border border-white/10 bg-black/20 p-3">
      <div className="flex items-center justify-between gap-2">
        ...
        <p className="text-sm font-semibold tabular-nums text-white">
          {hp}
          <span className="text-[11px] text-white/45">/{INITIAL_HP}</span>
        </p>
      </div>
...
function FireDamageBadge({ damage }) {
  ...
  return (
    <div className="relative flex items-center justify-center rounded-2xl border border-amber-200/35 bg-black/20 px-3 py-2">
      <div
      ...
      <div className="relative z-10 flex items-center gap-2">
        <span className="text-[10px] font-semibold uppercase tracking-[0.16em] text-amber-100/80">
          Damage
        </span>
...
export function GameHud({ redHp, blueHp, round, damage }) {
  return (
    <header className="rounded-3xl border border-white/10 bg-linear-to-r from-[#0f1726] via-[#151a2d] to-[#0f1726] p-4 shadow-[0_20px_40px_rgba(1,6,20,0.35)]">
      <div className="grid grid-cols-1 gap-3 lg:grid-cols-[1fr_auto_1fr]">
        <HealthTrack label="Red Agent" hp={redHp} tint="#f43f5e" />

        <div className="flex min-w-37.5 flex-col items-center justify-center rounded-2xl border border-white/10 bg-black/25 px-4 py-3">
          ...
          <div className="mt-1 flex items-center gap-1 text-white/65">
