...
export function gameReducer(state, action) {
  switch (action.type) {
    ...
    case "HIT": {
      ...
      if (player === "red") {
        if (forced && state.blue.stayed) {
          s.phase = "AwaitingFinalRevealVRF";
          s.active = null;
        } else if (forced) {
          s.phase = "AwaitingAction_Blue";
          s.active = "blue";
        } else {
          s.phase = "AwaitingAction_Red";
          s.active = "red";
        }
      } else {
        if (forced) {
    ...
    case "STAY": {
      ...
      return pushLog(
        s,
        `${player === "red" ? "Red" : "Blue"} stays at ${state[player].score}`,
      );
    }

    case "RIVER": {
      const rr = drawCard("rr"),
        br = drawCard("br");
      const rh = [...state.red.hand, rr],
        bh = [...state.blue.hand, br];
      const rs = calcScore(rh),
        bs = calcScore(bh);
      return pushLog(
        {
          ...state,
    ...
    case "RESOLVE": {
      const rd = absDiff21(state.red.score),
        bd = absDiff21(state.blue.score);
      const dmg = getDamage(state.round);

      if (rd === bd) {
        return pushLog(
          { ...state, phase: "AwaitingTiebreakerVRF", lastDmg: dmg },
    ...
    case "TIEBREAKER": {
      const rc = drawCard("tr"),
        bc = drawCard("tb");
      const rh = [...state.red.hand, rc],
        bh = [...state.blue.hand, bc];
      const rs = calcScore(rh),
        bs = calcScore(bh);
      const rd = absDiff21(rs),
        bd = absDiff21(bs);
      const dmg = state.lastDmg;
      const tied = rd === bd;
      const loser = tied ? null : rd > bd ? "red" : "blue";
      const nrh =
        loser === "red" ? Math.max(0, state.red.hp - dmg) : state.red.hp;
      const nbh =
        loser === "blue" ? Math.max(0, state.blue.hp - dmg) : state.blue.hp;
      ...
      return pushLog(
        {
          ...
          round:
            phase === "AwaitingInitialDeal" ? state.round + 1 : state.round,
          roundHistory: loser
            ? [
                ...state.roundHistory,
                {
...
export const PHASE_META = {
  ...
  AwaitingAction_Red: {
    label: "Red Agent's Turn",
    short: "RED TURN",
    color: "red",
  },
  AwaitingAction_Blue: {
    label: "Blue Agent's Turn",
    short: "BLUE TURN",
    color: "blue",
  },
  AwaitingFinalRevealVRF: {
    label: "River Incoming",
    short: "RIVER VRF",
    color: "purple",
  },
  ReadyToResolve: {
