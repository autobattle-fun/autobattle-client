...
export default function LudoGame() {
  ...
  const renderPawns = () => {
    ...
    COLORS.forEach((color) => {
      gameState.pawns[color].forEach((pawn, idx) => {
        ...
        // Position on 15x15 grid using percentages
        const cx = (gc - 0.5) * (100 / 15);
        const cy = (gr - 0.5) * (100 / 15);
        const pawnR = 2.2; // roughly 2.2% of board width
  ...
  return (
    <div className="w-full h-full relative bg-[#0f0f13] flex items-center justify-center overflow-hidden font-sans text-[#eee]">
      {/* Essential animation styles for the pawns */}
      <style
      ...
      <div className="absolute top-4 left-1/2 -translate-x-1/2 z-50 flex flex-col items-center gap-2 pointer-events-none">
        <h1 className="text-[22px] font-semibold tracking-[0.08em] text-white drop-shadow-md">
          LUDO
        </h1>
      ...
      {gameState.winner && (
        <div
          className="absolute top-20 z-50 text-lg font-bold px-6 py-3 rounded-xl text-center shadow-2xl backdrop-blur-md border-2"
          style={{
            color: "white",
            background: `${CHEX[gameState.winner]}dd`,
            borderColor: "white",
          }}
        >
      ...
      {/* ── BOARD CONTAINER ── */}
      <div className="relative aspect-square w-full h-full max-h-[90%] max-w-[90%] flex items-center justify-center pb-10">
        <div className="relative w-full h-full aspect-square">
          {/* The 2D SVG Background */}
          <svg
            viewBox="0 0 600 600"
            xmlns="http://www.w3.org/2000/svg"
            className="absolute inset-0 w-full h-full rounded-xl overflow-visible shadow-2xl z-0"
          >
      ...
      {/* ── DICE CONTROLS OVERLAY (Sticky at Bottom) ── */}
      <div className="absolute bottom-6 left-1/2 -translate-x-1/2 z-50 w-[min(90vw,350px)] pointer-events-auto">
        <div className="flex flex-col items-center gap-3 bg-black/60 backdrop-blur-md border border-white/10 rounded-[14px] px-5 py-4 shadow-2xl">
          <div
          ...
          <div className="flex items-center gap-4">
            <div className="w-14 h-14 bg-white rounded-[10px] grid grid-cols-3 grid-rows-3 p-2 gap-1 shadow-inner">
            ...
            <button
              ...
              className="px-6 py-2.5 rounded-lg border-none bg-[#4f46e5] hover:bg-[#6366f1] active:scale-95 text-white text-[15px] font-semibold cursor-pointer transition-all disabled:opacity-35 disabled:cursor-default shadow-md"
            >
              Roll Dice
            </button>
          </div>

          <div className="text-[13px] text-[#ccc] text-center min-h-[18px]">
            {gameState.msg}
          </div>
        </div>
      </div>
