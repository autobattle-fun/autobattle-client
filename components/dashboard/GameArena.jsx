    ...
    <div style={{ display: "flex", flexDirection: "column", gap: compact ? 3 : 5 }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
        ...
        <span style={{
          ...
          fontWeight: 900,
          color: crit ? "#ef4444" : "rgba(255,255,255,0.85)",
          fontVariantNumeric: "tabular-nums",
          letterSpacing: "-0.02em",
        }}>
          {hp}<span style={{ fontSize: compact ? 9 : 10, opacity: 0.4, fontWeight: 500 }}>/{INITIAL_HP}</span>
        </span>
      </div>
...
function PlayerZone({ player, ps, river, tbCards, isActive, flip = false }) {
  ...
  const mainCards = ps.hand.slice(0, ps.hand.length - (river ? 1 : 0) - tbCards.length);

  const statusLabel = bust ? "BUST" : is21 ? "21!" : ps.stayed ? "STOOD" : "";
  const statusColor = bust ? "#ef4444" : is21 ? "#fbbf24" : accentColor;

  const scoreColor = bust ? "#ef4444" : is21 ? "#fbbf24" : isActive ? accentColor : "rgba(255,255,255,0.9)";
    ...
    <div style={{
      ...
      <div style={{
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
          ...
          <div>
            <div style={{ fontSize: 13, fontWeight: 800, color: "rgba(255,255,255,0.85)", letterSpacing: "-0.01em" }}>
              {isRed ? "Red Agent" : "Blue Agent"}
            </div>
            {statusLabel && (
              <div style={{
      ...
      <div style={{
        ...
        {river && (
          <PlayingCard key={river.id} card={river} size="xl" isRiver />
        )}
        {tbCards.map(card => (
        ...
        {ps.hand.length === 0 && (
          <div style={{
            width: 80, height: 112, borderRadius: 9,
    ...
    <div style={{
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 10, flex: 1 }}>
        <div style={{
          ...
          animation: ["AwaitingAction_Red","AwaitingAction_Blue","AwaitingTiebreakerVRF"].includes(phase)
        ...
        <span style={{
          ...
          animation: phase === "AwaitingTiebreakerVRF" ? "pulse 1.2s infinite" : "none",
        }}>
          {meta.label}
        </span>
      </div>
      ...
      <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
        {phase === "AwaitingInitialDeal" && (
          <GhostButton onClick={() => dispatch({ type: "DEAL" })} accent="#e2e2e2" strong>
            Deal Cards
          </GhostButton>
        )}

        {(phase === "AwaitingAction_Red" || phase === "AwaitingAction_Blue") && active && (<>
          <GhostButton
        ...
        {phase === "AwaitingFinalRevealVRF" && (
          <GhostButton onClick={() => dispatch({ type: "RIVER" })} accent="#c084fc" pulse>
            Reveal River
          </GhostButton>
        )}

        {phase === "ReadyToResolve" && (
          <GhostButton onClick={() => dispatch({ type: "RESOLVE" })} accent="#fbbf24">
            Resolve Round
          </GhostButton>
        )}

        {phase === "AwaitingTiebreakerVRF" && (
          <GhostButton onClick={() => dispatch({ type: "TIEBREAKER" })} accent="#fde047" pulse>
            Draw Tiebreaker
          </GhostButton>
        )}

        {phase === "Ended" && !winner && (
          <GhostButton onClick={() => dispatch({ type: "RESET" })} accent="#e2e2e2">
            New Game
          </GhostButton>
        )}
      </div>
      ...
      <div style={{ display: "flex", gap: 20, alignItems: "center", flex: 1, justifyContent: "flex-end" }}>
        <div style={{ textAlign: "center" }}>
          <div style={{ fontSize: 9, color: "rgba(255,255,255,0.28)", textTransform: "uppercase", letterSpacing: "0.09em" }}>Round</div>
          <div style={{ fontSize: 16, fontWeight: 900, color: "rgba(255,255,255,0.7)", letterSpacing: "-0.02em", lineHeight: 1 }}>{round}</div>
        </div>
        <div style={{ width: 1, height: 24, background: "rgba(255,255,255,0.08)" }} />
        <div style={{ textAlign: "center" }}>
          <div style={{ fontSize: 9, color: "rgba(255,255,255,0.28)", textTransform: "uppercase", letterSpacing: "0.09em" }}>Damage</div>
          <div style={{ fontSize: 16, fontWeight: 900, color: "#fbbf24", letterSpacing: "-0.02em", lineHeight: 1 }}>{dmg}</div>
        </div>
      </div>
    </div>
  );
...
// GHOST BUTTON
// ─────────────────────────────────────────────────────────────
function GhostButton({ onClick, children, accent, pulse = false, strong = false }) {
...
function ScoreCompare({ red, blue }) {
  const rd = absDiff21(red.score), bd = absDiff21(blue.score);
  const tied = rd === bd;
  ...
  return (
    <div style={{
      ...
      <div style={{ flex: 1, display: "flex", alignItems: "center", gap: 10 }}>
        <span style={{ fontSize: 22, fontWeight: 900, color: redWins ? "#fb7185" : "rgba(255,255,255,0.2)", letterSpacing: "-0.03em", fontVariantNumeric: "tabular-nums" }}>
          {red.score}
        </span>
        <div>
          <div style={{ fontSize: 9, fontWeight: 700, color: redWins ? "#fb7185" : "rgba(255,255,255,0.2)", textTransform: "uppercase", letterSpacing: "0.1em" }}>
            {redWins ? "WINS" : tied ? "TIE" : "LOSES"}
          </div>
          <div style={{ fontSize: 9, color: "rgba(255,255,255,0.25)" }}>±{rd} from 21</div>
        </div>
      </div>

      <div style={{ fontSize: 10, fontWeight: 700, color: "rgba(255,255,255,0.18)", padding: "0 16px", textTransform: "uppercase", letterSpacing: "0.1em" }}>
        {tied ? "⚡ TIED" : "VS"}
      </div>

      <div style={{ flex: 1, display: "flex", alignItems: "center", gap: 10, justifyContent: "flex-end" }}>
        <div style={{ textAlign: "right" }}>
          <div style={{ fontSize: 9, fontWeight: 700, color: blueWins ? "#60a5fa" : "rgba(255,255,255,0.2)", textTransform: "uppercase", letterSpacing: "0.1em" }}>
            {blueWins ? "WINS" : tied ? "TIE" : "LOSES"}
          </div>
          <div style={{ fontSize: 9, color: "rgba(255,255,255,0.25)" }}>±{bd} from 21</div>
        </div>
        <span style={{ fontSize: 22, fontWeight: 900, color: blueWins ? "#60a5fa" : "rgba(255,255,255,0.2)", letterSpacing: "-0.03em", fontVariantNumeric: "tabular-nums" }}>
          {blue.score}
        </span>
      </div>
    </div>
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        padding: "44px 60px", borderRadius: 24,
        border: `1px solid ${color}28`,
        background: "rgba(10,10,16,0.95)",
        boxShadow: `0 0 80px ${color}18, 0 24px 64px rgba(0,0,0,0.5)`,
        transform: vis ? "scale(1) translateY(0)" : "scale(0.88) translateY(20px)",
        transition: "transform 0.5s cubic-bezier(0.34,1.4,0.64,1)",
      }}>
        <div style={{ fontSize: 52, lineHeight: 1 }}>🏆</div>
        <div style={{ textAlign: "center" }}>
          <div style={{ fontSize: 10, fontWeight: 700, color: "rgba(255,255,255,0.22)", textTransform: "uppercase", letterSpacing: "0.2em", marginBottom: 6 }}>
            VICTORY
          </div>
...
export function GameArena({ onGameEvent }) {
  ...
  const showCompare = ["ReadyToResolve", "AwaitingTiebreakerVRF", "Ended"].includes(phase)
    && red.hand.length > 0;
    ...
    <div style={{
      ...
      <div style={{
        ...
        <div style={{
          ...
          <span style={{ fontSize: 9, fontWeight: 700, color: "rgba(255,255,255,0.2)", textTransform: "uppercase", letterSpacing: "0.1em" }}>Round</span>
          <span style={{ fontSize: 20, fontWeight: 900, color: "rgba(255,255,255,0.6)", letterSpacing: "-0.03em", lineHeight: 1, fontVariantNumeric: "tabular-nums" }}>{round}</span>
        </div>
