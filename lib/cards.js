...
export function adaptServerStateToEngine(serverState) {
  if (!serverState || !serverState.red) return {};

  const createDeterministicCard = (serverCard, index, side) => {
    let rank;
    switch (serverCard.label) {
      case "1":
        rank = "A";
        break;
      case "11":
      ...
      case "13":
        ...
        break;
      default:
        rank = serverCard.label;
    }

    const stableId = `${serverState.gameId}-${serverState.roundNumber}-${side}-${index}-${rank}`;

    let hash = 0;
    for (let i = 0; i < stableId.length; i++) {
      hash = stableId.charCodeAt(i) + ((hash << 5) - hash);
    }
    const suitIndex = Math.abs(hash) % SUITS.length;
  ...
  const redCards = (serverState.red.cards || []).map((c, i) =>
    createDeterministicCard(c, i, "red"),
  );
  const blueCards = (serverState.blue.cards || []).map((c, i) =>
    createDeterministicCard(c, i, "blue"),
  );

  return {
    round: serverState.roundNumber,
    redHP: serverState.red.hp,
    blueHP: serverState.blue.hp,
    redCards,
    blueCards,
  };
}
